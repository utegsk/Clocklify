...
const promptQuestion = async ({ message, questionName, questionType, choices, validateFunction }) => {
  const defaultValidateQuestion = (value) => {
    if (value.length) {
      return true;
    } else {
      return "Wrong input";
  ...
  const validate = typeof validateFunction === "function" ? validateFunction : validateFunction === false ? () => true : defaultValidateQuestion
  const questions = [{
    name: questionName,
    type: questionType,
    choices,
