...
const { get: getFromStorage, set: saveToStorage } = require('./storage')
...
const stopWork = (argv) => {
  let endDate
  const _work = getFromStorage('work')
  if (argv.t && argv.d) {
  ...
  if (_work) {
    let _break = getFromStorage('break')
    if (_break.startAt) {
      pause.stop(_break)
    }
...
const toggleBreak = () => {
  if (getFromStorage('work')) {
    const _break = getFromStorage('break')
    if (_break.startAt) {
      pause.stop(_break)
    } else {
      pause.start(_break)
...
const monthGoal = (arg) => {
  let _goal = getFromStorage('goal')
  if (!_goal) {
    _goal = { active: false, workspaces: [] }
    saveToStorage('goal', _goal)
  }
  switch (arg) {
    case 'on':
