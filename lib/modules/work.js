...
const workLengthString = (startDate) => {
  ...
  return `${utils.formatTimeString(hours)} : ${utils.formatTimeString(minutes)} : ${utils.formatTimeString(seconds)}`
}
...
const stop = async (date) => {
  ...
  if (work) {
    if (work.lunchStart) {
      if (!work.lunchEnd) {
        utils.fprint(messages.BREAK_IN_PROGRESS, utils.messageType.ERROR)
      }
    }
    utils.fprint(workLengthString(work.startDate), utils.messageType.INFO)
  } else {
    utils.fprint(messages.NO_WORK_IN_PROGRESS, utils.messageType.WARNING)
    return
  }
  let userWorkspaces = await clockify.getWorkSpacesAndProjects()
  ...
  if (work || (typeof date === 'object' && typeof date !== 'null')) {
    ...
    utils.fprint(messages.NO_WORK_IN_PROGRESS, utils.messageType.ERROR)
  }
}
