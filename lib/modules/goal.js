...
const on = async (goal) => {
  if (goal.active) {
    ...
    setToStorage('goal', { ...goal, active: true })
...
const off = async (goal) => {
  if (goal.active) {
    setToStorage('goal', { ...goal, active: false })
...
const status = async (goal) => {
  if (goal.active) {
    ...
    for (const workspace of goal.workspaces) {
      ...
      const userFromResponse = response.groupOne.filter(user => user._id === getFromStorage('user').id)
...
const set = async (workspaceId = null) => {
  const goal = getFromStorage('goal')
...
const isWorkspaceGoalReached = async (workspaceId) => {
  ...
  const userFromResponse = response.groupOne.filter(user => user._id === getFromStorage('user').id)[0]
