...
const status = async (goal) => {
  if (goal.active) {
    ...
    for (const workspace of goal.workspaces) {
      ...
      const response = await clockify.getWorkingTimeFromRange(workspace.id, start, end)
...
const isWorkspaceGoalReached = async (workspaceId) => {
  ...
  const response = await clockify.getWorkingTimeFromRange(workspaceId, start, end)
