        ...
        project.children.forEach(dateEntry => {
            dateEntry.children.forEach(timeEntry => {
                ...
                parsedEntries.push([
                    project.name,
                    dateEntry.name,
                    `${formatTimeString(hours + days * 24)} : ${formatTimeString(minutes)} : ${formatTimeString(seconds)}`,
                    timeEntry.name
                ])
            })
        })
    })
    const { seconds, minutes, hours, days } = secondsToTimeIntervals(sum)
    const sumString = `${formatTimeString(hours + days * 24)} : ${formatTimeString(minutes)} : ${formatTimeString(seconds)}`
    createTable({
