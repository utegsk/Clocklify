...
const logWork = async (startDate, endDate) => {
    ...
    const userWorkspaces = (await findUserWorkspaces()).map(item => {
        item.value = {
            id: item.id,
        }
        return item
    })
    const { workspace: selectedWorkspace } = userWorkspaces.length === 1 ? { id: userWorkspaces[0].id } : await promptQuestion({
        questionName: "workspace",
        message: "Choose your workspace",
        questionType: "list",
        choices: userWorkspaces,
