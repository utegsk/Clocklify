...
const { set: setToStorage } = require('../storage')
const utils = require('../utils')


const start = (_break) => {
  const now = new Date()
  setToStorage('break', { ..._break, startAt: now })
...
const stop = (_break) => {
  const now = new Date()
  const currentBreakLength = now.getTime() - Date.parse(_break.startAt)
  setToStorage('break', { ..._break, total: _break.total + currentBreakLength, startAt: null })
